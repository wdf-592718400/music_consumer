<template>
  <div class="table">
    ...
    >
      ...
      <el-table-column label="歌曲图片" width="110" align="center">
        <template slot-scope="scope">
          ...
          <div class="play" @click="setSongUrl(scope.row.url, scope.row.name, scope.row.id)">
              <div v-if="toggle == scope.row.id && isPlay">
              <svg class="icon">
      ...
      <el-table-column label="资源更新" align="center" width="150">
        <template slot-scope="scope">
          ...
          <el-upload
            ...
            :on-success="handleSongAvatorSuccess"
            :before-upload="beforeSongAvatorUpload"
          >
    ...
    <el-dialog
      ...
      center
    >
      <el-form action="" :model="registerForm" id="addForm">
        <div>
          <label>歌名</label>
    ...
    >
      <el-form :model="form" ref="form" label-width="80px">
        <el-form-item prop="name" label="歌手-歌名" size="mini">
          <el-input v-model="form.name" placeholder="输入歌手和歌名"></el-input>
        </el-form-item>
...
import { songOfSingerId, updateSong, delSong } from "../api/index.js";
export default {
  mixins: [mixin],
  data() {
    return {
  ...
  methods: {
    ...
    // 上传歌曲之前的校验
        beforeSongAvatorUpload(file) {
            ...
            if (!extension) {
                this.$message.error('上传头像只能是 mp3 或 flac 格式!');
                return false;
            }
            const isLt2M = (file.size / 1024 / 1024) <= 100;
            if (!isLt2M) {
                this.$message.error('上传头像大小不能超过 100 MB');
                return false;
            }
            return true;
        },
        // 上传歌曲成功之后
        handleSongAvatorSuccess(res, file) {
            let _this = this;
            if (res.code == 1) {
                _this.getData();
                _this.$notify({
